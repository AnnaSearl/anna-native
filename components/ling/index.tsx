...
const Ling: React.ForwardRefRenderFunction<any, LingProps> = (props: LingProps, ref) => {
  ...
  const lingStyle = [
    styles[prefixCls],
    show ? styles[`${prefixCls}-show`] : null,
    position ? styles[`${prefixCls}-${position}`] : null,
  ];
  ...
  return (
    <View style={lingStyle}>
      <View style={styles[`${prefixCls}-container`]}>
        <View style={[styles[`${prefixCls}-title`], { color: config?.titleColor }]}>
          {config?.title}
        </View>
        {config?.content || config?.content === 0 ? (
          <View style={styles[`${prefixCls}-content`]}>{config?.content}</View>
        ) : null}
      </View>
      <View style={styles[`${prefixCls}-icon`]}>{iconNode}</View>
    </View>
  );
};
